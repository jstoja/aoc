...
loop do
  ...
  carts.collect { |c| [c[:y], c[:x]] }.sort.each do |y, x|
    ...
    if curr_cart.length > 1
      old_parts = curr_cart.reject { |c| CART_POSITIONS.include? c[:prev_part] }
      map[y][x] = old_parts.first[:prev_part]
      carts.select { |c| (c[:x].eql? x) && (c[:y].eql? y) }.each do |c|
        c[:deleted] = true
      end
      remaining_carts = carts.reject { |c| c[:deleted] }
      if remaining_carts.length == 1
        remaining_cart = remaining_carts.first
        puts "#{remaining_cart[:x]},#{remaining_cart[:y]}"
        exit
      end
  ...
  # At the end of each pass, we remove all the crashed carts
  carts = carts.delete_if { |c| c[:deleted] }
end
